...
`--date` の例は、日付のみなら `2026-08-31`、UTC の日時なら `2026-08-31T15:00:00Z`、日本標準時の日時なら `2026-09-01T00:00:00+09:00` となります。入力した日付やタイムゾーンは変換せずに出力されます。
...
| `toc.entries[].children` | 任意 | 目次項目の配列 | 子項目。同じ `label`、`page`、`children` の構造を指定可能 |

`output` と `images.directory` の相対パスは、設定ファイル自身の親ディレクトリを基準に解決されます。例えば `config/book.yaml` 内の `./images` は `config/images` を指します。

`images.order` を指定した場合は、列挙した画像だけを記述順で使用します。省略した場合は、`images.directory` 内の対応画像を自然順で使用します。空の配列、同じ画像の重複、存在しない画像または対応していない画像の指定はエラーになります。
...
`page: 0`、画像数を超えるページ番号、同じページ番号への重複指定はエラーになります。ページ配置の上書きは YAML 設定ファイルだけで指定できます。

目次項目は、各階層で YAML に記述した順序のままビューアーの目次へ出力します。`children` に同じ構造の項目を指定すると、階層化した目次を生成できます。ページ番号は、ページ配置と同じく `images.order` または自然順で画像を並べた後の 1 始まりの番号です。ラベルの重複と、複数項目から同じページへのリンクは指定可能です。

`toc` を省略した場合、または `toc.entries` を空にした場合は、書籍タイトルをラベルとして第 1 ページへリンクします。目次項目を指定した場合は、指定した項目だけを出力します。すべての階層で `label` と `page` は必須です。空または空白だけのラベル、`page: 0`、画像数を超えるページ番号はエラーになります。

目次項目は YAML 設定ファイルだけで指定できます。

未知のキーはエラーになります。現在は `layout` の指定を受け付けていません。
